...
;if ( X[i] == ODD )
	...
	AND	R10,R4,R1	;R10 = bitwise AND on X[i] and constant 1, returns zero for even
...
;then increment oddcount
	...
	JUMP	incr[R0]	;goto bottom of loop, do not perform operations for negatives

;else (on first if) incremenent negcount

else	ADD	R7,R7,R1	;negcount++ 

;no goto bottom needed after negcount++ as it is last operation before bottom
;if there were more, would require goto here.
...
X	DATA	0;	;X[0]  = 0
	...
	DATA	0;	;X[8]  = 0
	DATA	0;	;X[9]  = 0
possum	DATA	0	;initalise sum of positives to zero
